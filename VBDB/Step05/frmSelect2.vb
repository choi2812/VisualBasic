Public Class frmSelect2
    Dim Connection As New SqlClient.SqlConnection(My.Settings.DBConnectionString)
    Dim command As SqlClient.SqlCommand = Connection.CreateCommand
    Dim dr As SqlClient.SqlDataReader

    Private Sub frmSelect2_Load(sender As System.Object, e As System.EventArgs) Handles MyBase.Load
        Connection.Open()
    ...
    Private Sub btnSerch_Click(sender As System.Object, e As System.EventArgs) Handles btnSerch.Click
        ...
        Else
            If cmbBusho.Text = "すべて" Then
                If chkTaishoku.Checked = False Then
                    command.CommandText = "SELECT * FROM VB_Staff WHERE S_Resignflag = 0 "
                Else
                    command.CommandText = "SELECT * FROM VB_Staff WHERE S_Resignflag = 1"
                End If
            Else
                If chkTaishoku.Checked = False Then
                    command.CommandText = "SELECT * FROM VB_Staff WHERE S_Secton =" & "'" & cmbBusho.Text & "' and  S_Resignflag = 0"
                Else
                    command.CommandText = "SELECT * FROM VB_Staff WHERE S_Secton =" & "'" & cmbBusho.Text & "' and  S_Resignflag = 1"
                End If
            End If
        End If
        Try
